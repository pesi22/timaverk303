...
class PlaceDB(DbConnector):
    ...
    def add_place(self, name, region, population, web, latitude, longitude):
        new_id = 0
    ...
    def update_place(self, name, region, population, web, coord=None):
        rows_affected = 0
        result = self.execute_procedure('UpdatePlace', [name, coord, region, population, web])
        if result:
...
class HotelDB(DbConnector):
    ...
    def add_hotel(self, name, address, manager, web, email, phone, dist, lat=None, long=None):
        new_id = 0
        result = self.execute_procedure('AddHotel',[name, address, lat, long, manager, web, email, phone, dist])
        if result:
            new_id = int(result[0][0])
        return new_id
